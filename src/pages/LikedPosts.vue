...
export default {
  ...
  setup() {
    ...
    const sortedPost = ref([]); // 정렬된 게시글 목록
    ...
    const sortPosts = () => {
      if (selectedSort.value === 'popularity') {
        sortedPost.value = [...visibleDatas.value].sort((a, b) => b.likes - a.likes); // 좋아요 수 기준
      } else if (selectedSort.value === 'latest') {
        sortedPost.value = [...visibleDatas.value].sort((a, b) => new Date(b.createdDate) - new Date(a.createdDate)); // 최신순
      } else if (selectedSort.value === 'oldest') {
        sortedPost.value = [...visibleDatas.value].sort((a, b) => new Date(a.createdDate) - new Date(b.createdDate)); // 오래된순
      }
    };
